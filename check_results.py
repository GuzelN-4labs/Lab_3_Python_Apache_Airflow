...
Original file is located at
    https://colab.research.google.com/drive/16K7x86bZ9Caq4SG1Qat7qHjwTQRAsGr_
"""
...
DB_PATH = 'blog_posts_likes_comments_analysis.db'
CONTAINER_DB_PATH = '/opt/airflow/blog_posts_likes_comments_analysis.db'
...
def check_database():
    ...
    try:
        ...
        if ('blog_posts_analysis_by_topics',) not in tables:
            print("\nТаблица 'blog_posts_analysis_by_topics' не найдена!")
            return
        ...
        print(f"\nРезультаты расчета среднего количества лайков и комментариев на пост для каждой темы:")
        print("=" * 80)
        print(df.to_string(index=False))

        # Общая статистика
        total_topics = df['topic'].count()

        # создание вспомогательного DataFrame для допрасчетов, чтобы не изменять финальный датасет с результатами
        assist_df = result_df.copy()
        assist_df['total_avg_likes']=assist_df['avg_likes']*assist_df['total_posts']
        assist_df['total_avg_comments']=assist_df['avg_comments']*assist_df['total_posts']
        ...
        print(f"\nОбщая статистика:")
        print(f"  Общее количество топиков (тем): {total_topics:,}")
        print(f"  Среднее количество постов в теме: {avg_posts:,}")
        print(f"  Среднее количество лайков по всем темам: {avg_likes:,}")
        print(f"  Среднее количество комментариев по всем темам: {avg_comments:,}")


        # Лучшая и худшая категории
        best_topic = df.iloc[0]
        worst_topic= df.iloc[-1]

        print(f"\nЛучший показатель удержания:")
        print(f"  {best_topic['topic']}: {best_topic['avg_likes']:,}")

        print(f"\nТребует внимания:")
        print(f"  {worst_topic['topic']}: {worst_topic['avg_likes']:,}")
...
def copy_result_files():
    ...
    files_to_copy = [
        ('/opt/airflow/blog_posts_analysis_report.txt', 'blog_posts_analysis_report.txt'),
        ('/opt/airflow/blog_posts_analysis_data.csv', 'blog_posts_analysis_data.csv')
    ]
...
Файлы результатов:
    - blog_posts_analysis_report.txt            # Подробный текстовый отчет
    - blog_posts_analysis_data.csv              # Данные в формате CSV
    - blog_posts_likes_comments_analysis.db     # База данных SQLite
