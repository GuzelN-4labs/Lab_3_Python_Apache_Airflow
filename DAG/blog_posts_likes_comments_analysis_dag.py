...
def load_to_database(**context):
    ...
    try:
        ...
        ['topic', 'total_posts', 'avg_likes', 'avg_comments', 'avg_comments_wo_likes']
        try:
...
Общее количество топиков: {len(blog_posts_analysis_by_topics_df)}
...
- Доля пользователей оставивших комментарии, но не поставивиших лайк, в общем количестве комментариев: {row['ccomments_wo_likes_share']*100:.2f}%
"""
...
extract_blog_posts_task = PythonOperator(
    ...
    python_callable = blog_posts_data,
    dag=dag,
    doc_md="""
...
extract_comments_w_likes_task = PythonOperator(
    ...
    python_callable = comments_w_likes_data,
    dag=dag,
    doc_md="""
...
extract_comments_wo_likes_task = PythonOperator(
    ...
    python_callable = comments_wo_likes_data,
    dag=dag,
    doc_md="""
